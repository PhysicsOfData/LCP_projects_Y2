...
def plot_network(nodes,sphere=False,r_max=0):
    ...
    ax = Axes3D(fig)
    ax.scatter(nodes[1:,0],nodes[1:,1],nodes[1:,2],color="cyan")
    ax.scatter(nodes[0,0],nodes[0,1],nodes[0,2],color="cyan",s=200,alpha=0.8)   #Earth
    r = r_max
    ...
    if sphere:
        ...
        z = r*np.cos(v)
        ax.plot_wireframe(x, y, z, color=(88/255,88/255,88/255),alpha = 0.2,linewidths=1)
...
def disable_links(A,total_time,delta_t,mode="light",priority="random", custom_n_targets = None, custom_offtimes = None):
    ...
    if mode == "light":  
        ...
        rate = possible_links.shape[0] * 0.3  #disable 30% of possible links every time_max 
